            ...
            def L():
                ...
                if use_baseline:
                    ...
                    return (disc * rets * torch.exp(
                                distb.log_prob(acts)
                                - old_distb.log_prob(acts).detach()
                            )).mean()#The rets here actually means: use Gt to estimate the Q function. In the original text, it is the Q function. This approximation is reinforce. See the webpagehttps://sirlis.cn/posts/reinforcement-learning-Policy-Gradient/
